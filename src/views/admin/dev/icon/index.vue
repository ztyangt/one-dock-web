<template>
  <a-card class="dev-icons-page base-card cus-box-card" :bordered="false">
    <template #title>
      <div class="flex-yc g-20 py-5">
        <a-input @keyup.enter="searchIcon()" v-model="state.search" placeholder="输入图标名称..." style="max-width: 400px" />
        <a-button type="primary" @click="searchIcon">查询图标</a-button>
      </div>
    </template>
    ...
    <div v-if="icons.length">
      <a-space :size="20" wrap>
        <a-tooltip v-for="(item, index) in icons" :key="index" :content="item" placement="top">
          <a-card class="pointer base-card p-0 flex-center" @click="handleCopy(item, $event)">
