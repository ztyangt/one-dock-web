<template>
  <div class="flex-column h-100 p-20">
    <div>
      <div class="flex-yc g-20 py-5" style="display: flex; gap: 20px; align-items: center">
        <InputText
        ...
        />
        <Button label="查询图标" @click="searchIcon" />
      </div>
    </div>

    <div v-if="icons.length" class="flex-1 h-0 flex-column">
      <ScrollPanel class="flex-1 h-0">
        <div style="display: flex; flex-wrap: wrap; gap: 20px">
          <!-- 替代内层的 a-tooltip 和 a-card -->
          <div
            v-for="(item, index) in icons"
            :key="index"
            class="pointer base-card flex-center cursor-pointer border-round transition-all transition-duration-200 hover:surface-hover"
            v-tooltip.top="item"
      ...
      <div class="flex-end mt-4" style="display: flex; justify-content: flex-end; margin-top: 20px">
        <Paginator
          v-model:first="first"
          :rows="state.limit"
    ...
    <!-- 替代 a-empty -->
    <div v-else style="padding: 60px 0; text-align: center; color: var(--text-color-secondary)">
      <i class="pi pi-inbox" style="font-size: 3rem; margin-bottom: 10px; opacity: 0.5"></i>
      <div>暂无对应图标</div>
    </div>
  </div>
</template>
...
const getIconList = () => {
  const data = state.search ? iconList.filter((e) => e.includes(state.search)) : iconList
  const startIndex = first.value
  const endIndex = startIndex + state.limit
  state.length = data.length
  icons.value = data.slice(startIndex, endIndex)
}
